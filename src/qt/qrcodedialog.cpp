...
{
    QString ret = QString("solariscoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non XLR unit input in lnReqAmount, we generate the URI with XLR as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(solariscoinUnits::format(solariscoinUnits::XLR, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
